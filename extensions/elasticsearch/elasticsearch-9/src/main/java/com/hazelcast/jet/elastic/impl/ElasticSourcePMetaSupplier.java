...
public class ElasticSourcePMetaSupplier<T> implements ProcessorMetaSupplier {
    ...
    public void init(@Nonnull Context context) throws Exception {
        SearchRequest searchRequest = configuration.searchRequestFn().get().build();
        String[] indices = searchRequest.index().toArray(String[]::new);

        try (RestClient restClient = configuration.clientFn().get().build();
             ElasticCatClient catClient = new ElasticCatClient(restClient, configuration.retries())) {
            List<Shard> shards = catClient.shards(indices);
    ...
    static Map<Address, List<Shard>> assignShards(Collection<Shard> shards, Collection<Address> addresses) {
        Map<String, List<String>> assignment = addresses.stream()
                .map(Address::getHost).distinct().collect(toMap(identity(), a -> new ArrayList<>()));
        Map<String, List<String>> nodeCandidates = shards.stream()
